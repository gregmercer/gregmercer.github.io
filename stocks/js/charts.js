...
chartsModule.service('chartsService', function($http, $q) {
  ...
  this.refreshCharts = function(event, symbol) {

    var deferred = $q.defer();
    var that = this;

    this.refreshChartsList(event, symbol)
  ...
  this.refreshChartsList = function(event, symbol) {

    var deferred = $q.defer();

    if (event.target.checked) {
      // checking, add charts for the symbol
      this.getChartsData(symbol)
  ...
  this.getChartsData = function(symbol) {
